...
try {
    # Create a hash of the build context to check if rebuild is needed
    Write-Host "[+] Checking build context for changes..." -ForegroundColor Cyan
    $buildFiles = @(
        "Dockerfile", 
        "multi_service_processor.py", 
        "requirements.txt",
        ".env"
    )
    $buildDirs = @("ailibs", "RAG", "Recommendation")
    
    $contextHash = ""
    foreach ($file in $buildFiles) {
        if (Test-Path $file) {
            $fileHash = Get-FileHash $file -Algorithm MD5 | Select-Object -ExpandProperty Hash
            $contextHash += $fileHash
        }
    }
    foreach ($dir in $buildDirs) {
        if (Test-Path $dir) {
            $dirFiles = Get-ChildItem -Path $dir -Recurse -File | ForEach-Object { Get-FileHash $_.FullName -Algorithm MD5 | Select-Object -ExpandProperty Hash }
            $contextHash += ($dirFiles -join "")
        }
    }
    $contextHashFinal = [System.BitConverter]::ToString([System.Security.Cryptography.MD5]::Create().ComputeHash([System.Text.Encoding]::UTF8.GetBytes($contextHash))).Replace("-", "").ToLower()
    
    # Check if we have a cached hash
    $cacheFile = ".docker-build-cache"
    $cachedHash = if (Test-Path $cacheFile) { Get-Content $cacheFile } else { "" }
    ...
    Write-Host "[+] Checking if container image already exists in registry..." -ForegroundColor Cyan
    $imageExists = az acr repository show --name $CONTAINER_REGISTRY --image "multi-ai-processor:latest" --query "name" --output tsv 2>$null
    
    $shouldBuild = $false
    if ($imageExists) {
        Write-Host "[+] Container image already exists in registry: $imageExists" -ForegroundColor Green
        
        # Check if build context has changed
        if ($contextHashFinal -eq $cachedHash) {
            Write-Host "[+] Build context unchanged - no rebuild needed." -ForegroundColor Green
        } else {
            Write-Host "[+] Build context changed - rebuild required." -ForegroundColor Yellow
            $shouldBuild = $true
        }
        
        # Check if image exists locally
        $localImage = docker images "$REGISTRY_SERVER/multi-ai-processor:latest" --format "{{.Repository}}:{{.Tag}}" 2>$null
        if (-not $localImage -and -not $shouldBuild) {
            Write-Host "[+] Pulling existing image from registry..." -ForegroundColor Cyan
            docker pull "$REGISTRY_SERVER/multi-ai-processor:latest"
            if ($LASTEXITCODE -ne 0) {
                Write-Host "[!] Failed to pull image, will rebuild..." -ForegroundColor Yellow
                $shouldBuild = $true
            }
        } else {
            Write-Host "[+] Image already available locally." -ForegroundColor Green
        }
    } else {
        Write-Host "[+] No existing image found in registry - build required." -ForegroundColor Yellow
        $shouldBuild = $true
    }
    ...
    if ($shouldBuild) {
        ...
        # Update cache
        $contextHashFinal | Out-File $cacheFile -Encoding utf8
        Write-Host "[+] Container image pushed successfully." -ForegroundColor Green
    } else {
        Write-Host "[+] Skipping build and push - using existing container image." -ForegroundColor Green
    }
}
finally {
    Pop-Location
